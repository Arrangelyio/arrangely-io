...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: expiredPayments, error: findError } = await supabaseClient.from('payments').select('id').eq('status', 'pending').eq("payment_type", "lesson").lt('expires_at', new Date().toISOString());
    if (findError) {
    ...
    console.error('Error in expire-music-lab-payments:', error);
    return new Response(JSON.stringify({
      error: error.message
    }), {
      headers: {
