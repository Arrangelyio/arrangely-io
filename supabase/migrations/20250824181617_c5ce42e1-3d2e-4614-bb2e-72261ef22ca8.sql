-- Ensure events infrastructure exists and seed the requested event
...
-- 3) Triggers (drop first to avoid conflicts)
...
BEGIN
  IF TG_OP = 'INSERT' AND NEW.status = 'confirmed' THEN
    UPDATE public.events SET current_registrations = current_registrations + 1 WHERE id = NEW.event_id;
  ...
  ELSIF TG_OP = 'DELETE' AND OLD.status = 'confirmed' THEN
    UPDATE public.events SET current_registrations = current_registrations - 1 WHERE id = OLD.event_id;
  END IF;
...
-- 4) Seed the requested event
INSERT INTO public.events (
  title, description, date, start_time, end_time, location, address, price, max_capacity, current_registrations, speaker_name, speaker_bio, status, is_production
) VALUES (
  'Grand Launching Event Arrangely',
  'Join us for the grand launching of Arrangely! Experience the future of worship music arrangement with live demonstrations and performances.',
  '2025-09-17', '16:00', '18:00',
  'Jakarta Convention Center', 'Jl. Gatot Subroto, Jakarta Pusat, DKI Jakarta',
  0, 200, 0,
  'Echa Soemantri & Friends, Barry Likumahuwa',
  'Special performances by Echa Soemantri & Friends and Barry Likumahuwa.',
  'active', true
)
