-- Fix the rate limiting database function by removing the problematic ON CONFLICT clause
-- and also add a unique constraint to support proper UPSERT operations

-- First, add a unique constraint to support the ON CONFLICT operation
...
-- Fix the check_database_rate_limit function to handle the UPSERT properly
